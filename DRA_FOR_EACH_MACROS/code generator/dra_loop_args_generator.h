...
int dra_generate_loop_args(char const *filename, size_t count)
{
	...
	fprintf(file,"%s%s",DRA_LARGS_HEADER_START,DRA_LARGS_FOR_EACH_DO_FIRST_LOOP);
	for(size_t i = 2; i <= count; ++i)
	{
		fprintf(file,DRA_LARGS_FOR_EACH_DO_NTH_LOOP,i,i-1);
	}
	
	fprintf(file,"%s%s",DRA_LARGS_FOR_EACH_DO_DEF,DRA_LARGS_FOR_EACH_GET_FIRST_LOOP);
	...
	fprintf(file,"%s%s",DRA_LARGS_FOR_EACH_GET_DEF,DRA_LARGS_COUNT_NARGS_IMPL_START);
	...
	fprintf(file,"%s%s",DRA_LARGS_COUNT_NARGS_IMPL_END,DRA_LARGS_COUNT_NARGS_START);
	...
	fprintf(file,"%s%s",DRA_LARGS_COUNT_NARGS_END,DRA_LARGS_COUNT_NARGS_CALL_START);
	...
	fprintf(file,"%s%s",DRA_LARGS_COUNT_NARGS_CALL_END,DRA_LARGS_HEADER_END);
