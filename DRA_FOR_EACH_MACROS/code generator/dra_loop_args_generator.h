...
static char const DRA_EXPAND_MACRO[] = "#define DRA_EXPAND(x) x\n"; /* Exists only with the purpose of properly expanding __VA_ARGS__ in MSVC */
...
static char const DRA_LARGS_FOR_EACH_DO_NTH_LOOP[] = "#define FOR_EACH_DO_%d(fn,x,...) fn(x); DRA_EXPAND(FOR_EACH_DO_%d(fn,__VA_ARGS__))\n";

static char const DRA_LARGS_FOR_EACH_GET_FIRST_LOOP[] = "#define FOR_EACH_GET_1(fn,x) fn(x)\n";
static char const DRA_LARGS_FOR_EACH_GET_NTH_LOOP[] = "#define FOR_EACH_GET_%d(fn,x,...) fn(x) DRA_EXPAND(FOR_EACH_GET_%d(fn,__VA_ARGS__))\n";

static char const DRA_LARGS_FOR_EACH_ENUMERATE_FIRST_LOOP[] = "#define FOR_EACH_ENUMERATE_1(fn,x) fn(x)\n";
static char const DRA_LARGS_FOR_EACH_ENUMERATE_NTH_LOOP[] = "#define FOR_EACH_ENUMERATE_%d(fn,x,...) fn(x) , DRA_EXPAND(FOR_EACH_ENUMERATE_%d(fn,__VA_ARGS__))\n";

static char const DRA_LARGS_FOR_EACH_DO_DEF[] = "#define FOR_EACH_DO(fn,...) DRA_EXPAND(COUNT_NUM_ARGS_FN_CALL(FOR_EACH_DO,__VA_ARGS__) (fn,__VA_ARGS__))\n";
static char const DRA_LARGS_FOR_EACH_GET_DEF[] = "#define FOR_EACH_GET(fn,...) DRA_EXPAND(COUNT_NUM_ARGS_FN_CALL(FOR_EACH_GET,__VA_ARGS__) (fn,__VA_ARGS__))\n";
static char const DRA_LARGS_FOR_EACH_ENUMERATE_DEF[] = "#define FOR_EACH_ENUMERATE(fn,...) DRA_EXPAND(COUNT_NUM_ARGS_FN_CALL(FOR_EACH_ENUMERATE,__VA_ARGS__) (fn,__VA_ARGS__))\n";
...
static char const DRA_LARGS_COUNT_NARGS_CALL_START[] = "#define COUNT_NUM_ARGS_FN_CALL(func_name,...) DRA_EXPAND(COUNT_NUM_ARGS_IMPL(__VA_ARGS__";
...
int dra_generate_loop_args(char const *filename, size_t count)
{
	...
	fprintf(file,"%s",DRA_EXPAND_MACRO);
